...
class SiteController extends AdminBaseController
{
    ...
    {
        $this->pageTitle = 'Outreach Sites Management';
        $this->pageIcon = 'ti-share-alt';
        $this->totalSites = Site::all()->count();

        return $dataTable->render('outreachmanagement::admin.index', $this->data);
    }
    ...
    public function status(Request $request, Site $site)
    {
        try {
            ...
            //Add activity log
            $this->addLog($site->id, $request->status.' the website for outreach');

            //Add comment
            if ($request->message != null) {
                $this->addComment($site->id, $request->message);
            }
