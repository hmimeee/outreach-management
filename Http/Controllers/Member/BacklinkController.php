...
class BacklinkController extends MemberBaseController
{
    public $setting;
    ...
    public function index(BacklinksDataTable $dataTable)
    {
        ...
        return $dataTable->render('outreachmanagement::member.backlinks', $this->data);
    }
    ...
    {
        if (in_array(auth()->id(), $this->setting->observers) || in_array(auth()->id(), $this->setting->admins)){
            //
        } else {
            return Reply::error('You can not change the status!');
        }
